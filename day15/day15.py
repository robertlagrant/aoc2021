...
def generate_square_map(_raw, num_tiles=1):
    side = len(tile := [[int(c) for c in r] for r in _raw.split("\n")])
    return [
...
print(f"Part 1: {shortest_route(generate_square_map(raw))}")
print(f"Part 2: {shortest_route(generate_square_map(raw, num_tiles=5))}")
